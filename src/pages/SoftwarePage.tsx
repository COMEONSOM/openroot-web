...
export default function SoftwarePage() {
  ...
  // Optional video from softwareList — e.g. tool.video = "/videos/coevas-demo.mp4"
  const hasVideo = Boolean((tool as any).video);
    ...
    return hasVideo ? (
      <div className="sp-video-wrap">
        <div className="sp-video-frame">
          ...
          {/* ← Real HTML chrome bar — no more pseudo-element hacks */}
          ...
          <video
            ref={videoRef}
            src={(tool as any).video}
            autoPlay
            muted
            loop
            playsInline
            className="sp-video"
            aria-label={`${tool.name} demo video`}
          />
  ...
  return (
    <div className="sp-root">
      ...
      <header className={`sp-hero${hasVideo ? " sp-hero--split" : ""}`}>
        ...
        <div className="sp-hero-text">
          ...
          <div className="sp-hero-actions">
            <button className="sp-launch-btn" onClick={launch}>
            ...
            <p className="sp-launch-note">
              Login required · Secure access
            </p>
          </div>
        </div>

        {/* RIGHT — video on desktop (hidden on mobile, shown below) */}
        {hasVideo && (
      ...
      {/* Video on mobile — between hero and overview */}
      {hasVideo && (
